# Cairn Conformance Specification (v0.1.0-alpha.1)

This document is the authoritative list of invariants that every implementation of
[`cairn_core::VfsBackend`](https://docs.rs/cairn-core/0.1.0-alpha.1/cairn_core/trait.VfsBackend.html)
must satisfy in order to be considered compliant with the Cairn reference suite.

It pairs with the executable test suite published in
[`cairn-conformance`](https://docs.rs/cairn-conformance/0.1.0-alpha.1/cairn_conformance/).
...
1. `cairn_conformance::run(B::new())` returns without panicking when linked
   against `cairn-conformance = "X.Y.Z"`.
...
- **First-party backends** in this workspace
  (`cairn-backend-memory`, `cairn-backend-local`) run the suite from
  their own integration tests on every CI build.
- **Third-party backends** are encouraged to add `cairn-conformance` as a
  `dev-dependency` and call `cairn_conformance::run` from a `#[tokio::test]`.
...
Opens a handle on the node at `path` with the given [`OpenFlags`](https://docs.rs/cairn-core/0.1.0-alpha.1/cairn_core/struct.OpenFlags.html).
...
- **S2. A backend that exposes raw-symlink reads must document the API and
  return `NodeKind::Symlink` from that API.** (Cairn will likely add a
...
This specification is versioned with the `cairn-conformance` crate.
...
require a major version bump of `cairn-conformance` and a corresponding
...
If `cairn_conformance::run` panics on your backend:
...
3. If the assertion looks wrong (over-specified, platform-broken, or in
   ...
   [github.com/stonyp90/cairn](https://github.com/stonyp90/cairn)
   with the `conformance` label.

Genuine bugs in the suite (as opposed to bugs in backends it is testing) are
fixed in a patch release of `cairn-conformance` and noted in
`RELEASES.md`.
