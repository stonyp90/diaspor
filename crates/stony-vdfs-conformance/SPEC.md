# Stony VDFS Conformance Specification (v0.1.0-alpha.1)

This document is the authoritative list of invariants that every implementation of
[`stony_vdfs_core::VfsBackend`](https://docs.rs/stony-vdfs-core/0.1.0-alpha.1/stony_vdfs_core/trait.VfsBackend.html)
must satisfy in order to be considered compliant with the Stony VDFS reference suite.

It pairs with the executable test suite published in
[`stony-vdfs-conformance`](https://docs.rs/stony-vdfs-conformance/0.1.0-alpha.1/stony_vdfs_conformance/).
...
1. `stony_vdfs_conformance::run(B::new())` returns without panicking when linked
   against `stony-vdfs-conformance = "X.Y.Z"`.
...
- **First-party backends** in this workspace
  (`stony-vdfs-backend-memory`, `stony-vdfs-backend-local`) run the suite from
  their own integration tests on every CI build.
- **Third-party backends** are encouraged to add `stony-vdfs-conformance` as a
  `dev-dependency` and call `stony_vdfs_conformance::run` from a `#[tokio::test]`.
...
Opens a handle on the node at `path` with the given [`OpenFlags`](https://docs.rs/stony-vdfs-core/0.1.0-alpha.1/stony_vdfs_core/struct.OpenFlags.html).
...
- **S2. A backend that exposes raw-symlink reads must document the API and
  return `NodeKind::Symlink` from that API.** (Stony VDFS will likely add a
...
This specification is versioned with the `stony-vdfs-conformance` crate.
...
require a major version bump of `stony-vdfs-conformance` and a corresponding
...
If `stony_vdfs_conformance::run` panics on your backend:
...
3. If the assertion looks wrong (over-specified, platform-broken, or in
   ...
   [github.com/stonyp90/stony-vdfs](https://github.com/stonyp90/stony-vdfs)
   with the `conformance` label.

Genuine bugs in the suite (as opposed to bugs in backends it is testing) are
fixed in a patch release of `stony-vdfs-conformance` and noted in
`RELEASES.md`.
