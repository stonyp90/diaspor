...
`stony-vdfs` ships with a structural — not aspirational — privacy
...
- **Audit your sidecar destination.** If your VFS backend is
  `stony-vdfs-backend-cloud-s3` (hypothetical future backend), the
...
- The behaviour of third-party `Transcriber` / `Tagger` implementations
  in other crates. Authors of `stony-vdfs-transcribe-azure` etc. are
