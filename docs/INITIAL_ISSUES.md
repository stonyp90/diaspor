...
`github.com/stonyp90/diaspor` right after the initial push so the repo
...
The conformance suite in `diaspor-conformance` must exercise every
...
**Acceptance criteria:** a markdown spec in `crates/diaspor-conformance/SPEC.md`
...
`diaspor-backend-memory` currently has no xattr support. The trait
...
`diaspor-fuse` today is a stub. Wire it against the `fuser` crate
...
a memory backend at `/tmp/diaspor-demo` and `ls` works.
...
Replace the no-op probe in `diaspor-index` with a real
