...
The default `FfmpegExtractor` shipped in `stony-vdfs-index` calls **`ffmpeg` as a
...
can pin or upgrade `ffmpeg` independently of `stony-vdfs` releases. The build does not
