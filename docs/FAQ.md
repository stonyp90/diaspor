...
does one thing — move bytes — and does it well. `diaspor` is a *library*, not a CLI,
...
`diaspor` storage to an off-site backup. The two projects address different layers
of the stack.

### How is this different from Nextcloud?

Nextcloud is a self-hosted server with a web UI, sharing, calendars, contacts, and a
plugin marketplace. It is a full collaboration platform. `diaspor` is a Rust library
that gives a *single user's application* a filesystem-shaped API with local AI
indexing. The two could integrate — a Nextcloud-replacement app could use `diaspor`
as its local cache layer with offline transcription — but `diaspor` itself ships no
...
uploaded to their data centres for processing. `diaspor` is the open-source plumbing
...
respective operating systems. `diaspor` is cross-platform (Linux, macOS, Windows),
open-source (MIT), and the indexing models are user-chosen. An application built on
`diaspor` can ship the same on-device feature across all three desktops with a
...
build a multi-user application *on top of* `diaspor`. The library happily runs
...
own, so accessibility applies to downstream applications. Output from `diaspor-cli`
...
### Is there a trademark on "diaspor"? Can I fork it?
...
`diaspor` does not bundle any of them, so the obligation is on the integrator.
