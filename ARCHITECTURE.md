...
This document explains the design choices behind `diaspor` and how the crates fit
...
```mermaid
graph TD
    core["diaspor-core<br/><sub>traits / paths / errors</sub>"]
    memory["diaspor-backend-memory"]
    local["diaspor-backend-local"]
    fuse["diaspor-fuse<br/><sub>Linux/macOS</sub>"]
    winfsp["diaspor-winfsp<br/><sub>Windows</sub>"]
    index["diaspor-index<br/><sub>FFmpeg + Whisper + LLM</sub>"]
    cli["diaspor-cli"]
...
5. **Small core.** `diaspor-core` depends on the minimum: `thiserror`, `bytes`,
...
`Result<T> = std::result::Result<T, VfsError>` is exported from `diaspor-core` and
...
graph LR
    ...
    fuse["FUSE adapter<br/><sub>diaspor-fuse</sub>"]
    winfsp["WinFsp adapter<br/><sub>diaspor-winfsp</sub>"]
...
- **Content-addressable storage.** The `VfsBackend` trait does not expose hashes today,
  but `diaspor-cas` (M5) will provide an additional trait that CAS-aware backends
