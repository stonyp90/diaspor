...
///
/// This factory trait allows the application layer to create cache adapters
/// without depending on concrete implementations, following the Dependency
/// Inversion Principle.
#[async_trait]
pub trait CacheAdapterFactory: Send + Sync {
    /// Create a cache adapter from configuration
    ///
    /// Returns an `Arc<dyn CacheAdapter>` to allow sharing across threads
