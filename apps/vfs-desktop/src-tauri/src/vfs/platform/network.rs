...
pub async fn mount_smb_share(
    ...
    // Capture drive_letter before moving cmd into closure
    let drive_letter_clone = drive_letter;
    let unc_path_owned = unc_path.to_string();
    
    // Build args vector for Windows-safe command execution
    let mut args: Vec<String> = vec!["use".to_string()];
    
    if let Some(letter) = drive_letter {
        args.push(format!("{}:", letter));
    } else {
        args.push("*".to_string()); // Auto-assign drive letter
    }
    
    args.push(unc_path_owned.clone());
    ...
    if output.status.success() {
        ...
        if let Some(letter) = drive_letter_clone {
            Ok(PathBuf::from(format!("{}:\\", letter)))
        } else {
            // Try to extract from output
            debug!("SMB mount output: {}", stdout);
