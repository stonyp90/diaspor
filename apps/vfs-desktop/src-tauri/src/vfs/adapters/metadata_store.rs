...
impl IMetadataStore for JsonMetadataStore {
    ...
    async fn add_tag(&self, source_id: &str, path: &Path, tag: FileTag) -> Result<()> {
        ...
        // Remove existing tag with same name (to update color if changed)
        metadata.tags.retain(|t| t.name != tag.name);
        
        // Add the new tag (with updated color if provided)
    ...
    async fn list_all_tags(&self, source_id: &str) -> Result<Vec<FileTag>> {
        ...
        // Collect all tags with their colors
        let mut tag_map: std::collections::HashMap<String, FileTag> = std::collections::HashMap::new();
        
        for (_, metadata) in cache.iter().filter(|(k, _)| k.starts_with(&prefix)) {
            for tag in &metadata.tags {
                // If tag already exists, prefer the one with a color (or keep existing if both have colors)
                let existing = tag_map.get(&tag.name);
                match existing {
                    Some(existing_tag) => {
                        // If new tag has color and existing doesn't, or both have colors, use new one
                        if tag.color.is_some() && (existing_tag.color.is_none() || existing_tag.color.is_some()) {
                            tag_map.insert(tag.name.clone(), tag.clone());
                        }
                    }
                    None => {
                        tag_map.insert(tag.name.clone(), tag.clone());
        ...
        let mut all_tags: Vec<FileTag> = tag_map.into_values().collect();
...
impl JsonMetadataStore {
    /// Remove all metadata entries for a specific source
    /// This is called when a storage source is removed
    pub async fn remove_source_metadata(&self, source_id: &str) -> Result<usize> {
        let prefix = format!("{}:", source_id);
        let mut cache = self.cache.write().await;
        ...
        if count > 0 {
            self.mark_dirty().await;
            self.save().await?;
            info!("Removed {} metadata entries for source: {}", count, source_id);
        }
