    ...
    async fn list_files(&self, path: &Path) -> Result<Vec<VirtualFile>> {
        ...
        for (idx, entry) in entries.iter().enumerate() {
            ...
            if child_name.ends_with(".part") 
                ...
                debug!("[GCS] Skipping temporary/chunk file: '{}' (entry: '{}')", child_name, entry_name);
                continue;
            }
            ...
            // Set last_modified from GCS metadata
...
mod tests {
    ...
    fn test_detect_tier_archive() {
        assert_eq!(detect_gcs_tier(Some("ARCHIVE")), StorageTier::Cold);
        assert_eq!(detect_gcs_tier(Some("COLDLINE")), StorageTier::Cold);
    }
    ...
    fn detect_gcs_tier(storage_class: Option<&str>) -> StorageTier {
        match storage_class {
            Some("ARCHIVE") | Some("COLDLINE") => StorageTier::Cold, // Archive maps to Cold
            _ => StorageTier::Cold,
        }
    }
}
