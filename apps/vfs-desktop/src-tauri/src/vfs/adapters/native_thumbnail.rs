...
impl NativeThumbnailAdapter {
    ...
    async fn check_availability() -> bool {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            cache_path.exists() || tokio::process::Command::new("which")
    ...
    async fn generate_linux_thumbnail(
        ...
        if thumb_type == ThumbnailType::Video {
            let status = tokio::process::Command::new("ffmpegthumbnailer")
                .args([
...
mod tests {
    ...
    async fn test_adapter_creation() {
        ...
        let _adapter = NativeThumbnailAdapter::new(temp_dir.path().to_path_buf())
            .await
            .unwrap();
