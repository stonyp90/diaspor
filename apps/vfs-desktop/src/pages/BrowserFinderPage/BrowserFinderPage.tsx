...
import { getBrowserApi } from '../../services/browser-api';
import type {
  StorageSource,
  FileMetadata,
  ApiSearchRequest,
...
export function BrowserFinderPage() {
  ...
  const handleAddTag = async (
    ...
    try {
      ...
      await api.addTags(selectedSource.id, [file.path], [tagName]);
      // Refresh tags
      loadTags();
                ...
                {displayFiles.map((file) => (
                  <div
                    ...
                    className={`file-item ${selectedFiles.has(file.path) ? 'selected' : ''} ${
                      file.tierStatus === 'cold' || false // Archive tier removed
                        ? 'cold-file'
                        : ''
                    }`}
        ...
        {showInfoPanel && selectedFile && (
          <aside className="finder-info">
                    ...
                    {selectedFile.tags.map((tag) => {
                      ...
                      return (
                        <span
                          key={tagName}
                          className="meta-tag"
                          onClick={() => handleRemoveTag(selectedFile, tagName)}
            ...
            <div className="info-actions">
              ...
              {(selectedFile.tierStatus === 'cold' || false) && ( // Archive tier removed
                <button
