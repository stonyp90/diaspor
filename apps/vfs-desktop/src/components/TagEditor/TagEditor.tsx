...
export const TagEditor: React.FC<TagEditorProps> = ({
  ...
  const [suggestedTags, setSuggestedTags] = useState<FileTag[]>([]);
  ...
  // Load AI-suggested tags if it's a video file
  useEffect(() => {
    const loadSuggestions = async () => {
      if (
        !isVideoFile(fileName, mimeType) ||
        (fileCategory && !isMountedStorage(fileCategory))
      ) {
        return;
      }
      ...
      try {
        ...
        >('get_pending_tag_suggestions_for_file', {
          sourceId,
          filePath: path,
        });

        if (pending && pending.length > 0) {
          // Convert to FileTag format
          const tags: FileTag[] = pending[0].suggestedTags.map((t) => ({
            name: t.name,
            color: '#0a84ff', // Use primary color for AI suggestions
          }));
          setSuggestedTags(tags);
        }
      } catch (err) {
        // No suggestions available or command doesn't exist yet
        setSuggestedTags([]);
      }
    ...
    loadSuggestions();
  }, [sourceId, path, fileName, mimeType, fileCategory]);

  // Request AI tag suggestions
  const requestAISuggestions = async () => {
    if (!fileCategory || !isMountedStorage(fileCategory)) {
      return;
    }

    setLoadingSuggestions(true);
    try {
      await requestTagSuggestions(sourceId, path, fileName, fileCategory);
      ...
      console.error('Failed to request AI suggestions:', err);
      setLoadingSuggestions(false);
    }
  };
  ...
  const setColorLabel = async (color: string | null) => {
    try {
      await invoke('vfs_set_color_label', { source_id: sourceId, path, color });
    } catch (error) {
  ...
  const setRating = async (rating: number | null) => {
    try {
      await invoke('vfs_set_rating', { source_id: sourceId, path, rating });
    } catch (error) {
  ...
  return (
    <div className="tag-editor">
      ...
      <div className="tag-editor-section">
        ...
        {isVideoFile(fileName, mimeType) &&
          fileCategory &&
          isMountedStorage(fileCategory) && (
            <div className="tag-editor-ai-suggestions">
              <div className="tag-editor-ai-header">
                <Sparkles size={14} />
                <span className="tag-editor-suggestions-label">
                  AI Suggestions
                </span>
                {loadingSuggestions ? (
                  <span className="tag-editor-loading-text">Generating...</span>
                ) : (
                  <button
                    className="tag-editor-ai-refresh"
              ...
              {suggestedTags.length > 0 ? (
                <div className="tag-editor-suggestions">
                  {suggestedTags
                    .filter(
                      (t) => !metadata.tags.some((mt) => mt.name === t.name),
                    )
                    .map((tag) => (
                      <button
                        key={tag.name}
                        className="tag-editor-suggestion tag-editor-ai-tag"
                        onClick={() => addExistingTag(tag)}
                        style={{ borderColor: tag.color || '#0a84ff' }}
                      >
                        <span
                          className="tag-editor-tag-dot"
                ...
                !loadingSuggestions && (
                  <span className="tag-editor-no-suggestions">
                    No suggestions yet. Click "Generate" to create AI tags.
                  </span>
                )
              )}
            </div>
