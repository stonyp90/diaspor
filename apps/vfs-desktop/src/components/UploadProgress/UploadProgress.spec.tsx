...
describe('UploadProgress', () => {
  ...
  describe('Progress Display', () => {
    ...
    it('should display progress when data is available', async () => {
      ...
      const progressBar = screen
        .getByText(/50\.0/)
        .closest('.upload-progress')
        ?.querySelector('.upload-progress-bar');
      expect(progressBar).toBeInTheDocument();
      expect(progressBar).toHaveStyle({ width: '50%' });
    });

    it('should display 0% progress for new uploads', async () => {
      const zeroProgressData = {
        ...mockProgressData,
        bytes_uploaded: 0,
        percentage: 0,
      };
      mockInvoke.mockResolvedValue(zeroProgressData);
      render(
        <UploadProgress
          uploadId={uploadId}
          fileName="test-file.txt"
    ...
    it('should display progress updates during upload', async () => {
      ...
      // Trigger re-render by advancing time (component polls every 500ms)
  ...
  describe('Pause/Resume Functionality', () => {
    ...
    it('should call pause upload when Pause is clicked', async () => {
      ...
      const pauseButton = screen.getByLabelText(/Pause upload/i);
  ...
  describe('Cancel Functionality', () => {
    it('should show Cancel button', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByLabelText(/Cancel upload/i)).toBeInTheDocument();
      });
    });
  ...
  describe('Cleanup', () => {
    it('should clear interval on unmount', async () => {
      ...
      // Advance timer - should not trigger more calls after unmount
