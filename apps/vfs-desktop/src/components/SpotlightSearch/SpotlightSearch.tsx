...
// Platform detection for display (cached from Tauri backend)
const getPlatformInfo = () => getPlatformInfoSync();
const isMac = getPlatformInfo().isMac;
...
export function SpotlightSearch({
  ...
  const parseQuery = useCallback((q: string) => {
    ...
    const tagMatches = q.matchAll(/tag:(\S+)/gi);
  ...
  const results = useMemo((): SearchResult[] => {
    ...
    // Parse query for operators
    const { textSearch, tagFilters, typeFilter, extFilter } = parseQuery(query);

    // Search files and folders with operator-aware filtering
    const matchingFiles = files
      .filter((f) => {
        // Filter by tag operators (AND logic - file must have ALL specified tags)
        ...
        if (textSearch) {
          ...
          const tagMatch = (f.tags || []).some((t) => {
            const tagName = typeof t === 'string' ? t : t.name;
            return tagName.toLowerCase().includes(textSearch);
          });
    ...
    // Search tags - only show tags that aren't already filtered
    // Use textSearch for tag search, or the original query if no operators were parsed
    if (!tagFilters && textSearch) {
      availableTags
    ...
    } else if (!tagFilters && !typeFilter && !extFilter) {
    ...
    const hasOperator = tagFilters || typeFilter || extFilter;
    if (!hasOperator) {
  ...
  const handleSelect = useCallback(
    (result: SearchResult) => {
      switch (result.type) {
        ...
        case 'tag': {
          // Append tag to query if not already present
          const { tagFilters: currentTags } = parseQuery(query);
          const tagName = result.title.toLowerCase();
          if (!currentTags || !currentTags.includes(tagName)) {
            setQuery(`${query.trim()} tag:${result.title} `.trim());
          }
  ...
  return (
    <div className="spotlight-overlay" onClick={onClose}>
      <div className="spotlight-container" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Active tag filters */}
        {(() => {
          const { tagFilters: activeTags } = parseQuery(query);
                ...
                {activeTags.map((tag, index) => (
                  <button
                    ...
                    onClick={(e) => {
                      ...
                      const { tagFilters, textSearch } = parseQuery(query);
                      const newTags = tagFilters
                        ? tagFilters.filter((t) => t !== tag)
                        : [];
                      const newQuery =
                        newTags.length > 0
                          ? `${newTags.map((t) => `tag:${t}`).join(' ')} ${textSearch}`.trim()
                          : textSearch;
                      setQuery(newQuery);
                    }}
                  >
