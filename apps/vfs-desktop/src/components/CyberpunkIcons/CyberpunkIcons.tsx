...
// OS-agnostic: Covers standard user folders across macOS, Linux, and Windows
export function getFolderIcon(name: string): React.FC<IconProps> {
  const lowerName = name.toLowerCase();

  // Home directory variations (macOS/Linux/Windows)
  if (lowerName === 'home' || lowerName === 'user' || lowerName === 'users') {
    return IconHome;
  }

  // Desktop folder (consistent across all OS)
  if (lowerName === 'desktop') {
    return IconDesktop;
  }

  // Documents folder variations
  // Standard: "Documents" (macOS/Linux/Windows)
  // Windows legacy: "My Documents"
  if (lowerName === 'documents' || lowerName === 'my documents') {
    return IconDocuments;
  }

  // Downloads folder (consistent across all OS)
  if (lowerName === 'downloads') {
    return IconDownloads;
  }

  // Pictures/Photos folder variations
  // Standard: "Pictures" (macOS/Linux), "Photos" (macOS)
  // Windows: "My Pictures"
  if (
    lowerName === 'pictures' ||
    lowerName === 'photos' ||
    lowerName === 'my pictures'
  ) {
    return IconPictures;
  }

  // Music folder variations
  // Standard: "Music" (macOS/Linux/Windows)
  // Windows legacy: "My Music"
  if (lowerName === 'music' || lowerName === 'my music') {
    return IconMusic;
  }

  // Videos folder variations
  // Standard: "Videos" (macOS/Linux/Windows)
  // Windows legacy: "My Videos"
  if (lowerName === 'videos' || lowerName === 'my videos') {
    return IconVideo;
  }

  // Volumes (macOS: /Volumes, Linux: /media, Windows: drive letters)
  if (lowerName === 'volumes') {
    return IconVolumes;
  }
...
// Storage Tier Icons - declared here after main export
// Flame icon for Hot/Warm storage tier
export const IconFlame: React.FC<IconProps> = ({
  ...
  <svg
    className={`cyber-icon storage-tier-icon flame ${glow ? 'glow' : ''} ${className}`}
    width={size}
    height={size}
    viewBox="0 0 24 24"
    fill="none"
...
// Snowflake icon for Cold storage tier
export const IconSnowflake: React.FC<IconProps> = ({
  size = 16,
  color = 'currentColor',
  glow = true,
  className = '',
  ...
  <svg
    className={`cyber-icon storage-tier-icon snowflake ${glow ? 'glow' : ''} ${className}`}
    width={size}
    height={size}
    viewBox="0 0 24 24"
    fill="none"
  ...
  >
    ...
    <line x1="18.34" y1="5.66" x2="5.66" y2="18.34" />
...
// Clock icon for Nearline storage tier
export const IconClock: React.FC<IconProps> = ({
  ...
  <svg
    className={`cyber-icon storage-tier-icon clock ${glow ? 'glow' : ''} ${className}`}
    width={size}
    height={size}
    viewBox="0 0 24 24"
    fill="none"
...
// Bolt/Lightning icon for Instant Retrieval storage tier
export const IconBolt: React.FC<IconProps> = ({
  ...
  <svg
    className={`cyber-icon storage-tier-icon bolt ${glow ? 'glow' : ''} ${className}`}
    width={size}
    height={size}
    viewBox="0 0 24 24"
    fill="none"
...
export const IconComment: React.FC<IconProps> = ({
  ...
  >
    ...
    <line x1="8" y1="13" x2="14" y2="13" opacity="0.5" />
  </svg>
);
